...
import { saveFileToFtpServer } from "./FtpController";
...
class IpoDetailController {
    ...
    async getIpoDetail(req: Request, res: Response, next: NextFunction) {        
        try {
            ...
            const imageUrl = `/${logoPathLocal}/${ipoDetailsQuery?.CompanyLogo}`

            let anchorInvestorPdf = null;
            if(ipoDetailsQuery?.IpoDetail?.AnchorListLink) {
                anchorInvestorPdf = `/${anchorPathLocal}/${ipoDetailsQuery?.IpoDetail?.AnchorListLink}`
            }
            let boaImage = null            
            if(ipoDetailsQuery?.IpoDetail?.BasicOfAllotment) {
                boaImage = `/${boaPathLocal}/${ipoDetailsQuery?.IpoDetail?.BasicOfAllotment}`
            }
    ...
    async uploadLogo(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const response = await saveFileToFtpServer(req.file,ServerUploadFileEnum.LOGO)
            return res.redirect(`/ipoDetail/get/${IpoId}?success=${response}`);
        } catch (error) {
            console.error('Error updating timeline:', error);
            return res.status(500).send('Internal server error');
        }
    ...
    async uploadBoa(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            const response = await saveFileToFtpServer(req.file,ServerUploadFileEnum.BOA)
            return res.redirect(`/ipoDetail/get/${IpoId}?success=${response}`);
    ...
    async uploadAnchorInvestor(req: Request, res: Response, next: NextFunction) {
        ...
        const response = await saveFileToFtpServer(req.file,ServerUploadFileEnum.ANCHOR)
        return res.redirect(`/ipoDetail/get/${IpoId}?success=${response}`);
            // return res.redirect(`/ipoDetail/get/${IpoId}`);
        } catch (error) {
            console.error('Error updating timeline:', error.message);
