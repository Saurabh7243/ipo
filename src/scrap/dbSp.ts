...
async function DetailsInsertData(
  ...
  try {
    ...
    var sql = `INSERT INTO IpoDetails (
        IpoId, FaceValue, MinimumPrice, MaximumPrice, LotSize, 
...
async function DetailsUpdateData(
  ...
  try {
    ...
    // const validateIntRange = (value: number) => {
    //   if (value < -2147483648 || value > 2147483647) {
    //     throw new Error(`Value ${value} is out of int range.`);
    //   }
    //   return value;
    // }
    // validateIntRange(ShareHoldingPreIssue)
...
async function subscriptionInsertData(fields:subscriptionFields)
{
  ...
  var sql =""
  if(fields.Type == TypeEnum.Mainbord) {

    sql = `INSERT INTO IpoSubscriptions
    (
      IpoId,
      SniiSubscription,
  ...
  } else if(fields.Type == TypeEnum.SME) {
    sql = `INSERT INTO IpoSubscriptions
    (
      IpoId,
      SniiSubscription,
...
async function subscriptionUpdateData(fields:subscriptionFields)
{
  ...
  if(fields.Type == TypeEnum.Mainbord) {
    ...
    sql = `UPDATE IpoSubscriptions SET 
...
//ipo subscription insertUpdateAppwise start //
export async function subscriptionAppWiseInsertUpdate(fields:subscriptionFields){
  ...
  if (result && result.length > 0) {
      console.log('Row was found!');
      subscriptionAppWiseUpdateData(fields);
  } else {
      console.log('Row was not found :( ! )');
      subscriptionAppWiseInsertData(fields);
  }
}

async function subscriptionAppWiseInsertData(fields:subscriptionFields)
{
  if (fields.RetailApplication == 0) return;
  if (fields.RetailApplication == null) return;
...
async function subscriptionAppWiseUpdateData(fields:subscriptionFields)
{
  if (fields.RetailApplication == 0) return;
  if (fields.RetailApplication == null) return;
...
//ipo subscription insertUpdateAppwise end //

// ipo noReservationsInsertUpdate start //
...
// ipo noReservationsInsertUpdate end //


// ipo DetailAboutInsertUpdate start //
export async function DetailAboutInsertUpdate(
  IpoId: number,
  about_data: any
) {
  if (about_data.toString().trim() == "") return;
  ...
  try {
    if (result && result.length > 0) {
      console.log("Row was found!");
      DetailAboutUpdateData(IpoId, about_data);
    } else {
      console.log("Row was not found :( !");
      DetailAboutInsertData(IpoId, about_data);
    }
  } catch (error) {
    console.log(error);
  }
...
async function DetailAboutInsertData(IpoId: number, about_data: any) {
  try {
    var sql =`INSERT INTO IpoDetails (IpoId, objectives) VALUES (${IpoId}, '${about_data}')`;
    const connection = await dbUtils.getDefaultConnection();
    const result = await connection.query(sql);
  } catch (error) {
    console.log(error);
...
async function DetailAboutUpdateData(IpoId: number, about_data: any) {
  try {
...
// ipo DetailAboutInsertUpdate end //