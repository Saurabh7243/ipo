...
export async function ipoDetailScrap(IpoId: number, chittorid: number, chittorname: string) {
    // 
    // console.log(`Processing IPO with IpoId -------============-----------> :`, IpoId,chittorid,chittorname);
    ...
    try {
        ...
        if(freshIssuePrice == '₹[.] Cr') {
            freshIssuePrice = ""
        }

        if(totalIssuePrice == '₹[.] Cr') {
            totalIssuePrice = ""
        }
       ...
       const datxx = await DetailsInsertUpdate(IpoId,faceValue,minPrice,maxPrice,lotSize,shareHoldingPreIssue,
